<?php 
	...
	class M_san_pham extends database
	{
		...
		{
			$sql = "SELECT * FROM san_pham where ten_san_pham like '%$gt%'";
			$this->setQuery($sql);
			return $this->loadAllRows();	
		}
		...
		{	
			if($vt>=0 && $limit>0)		
				$sql = "select * from san_pham where ma_loai_cha=9 limit $vt,$limit";
			else 
		...
		{	
			if($vt>=0 && $limit>0)		
				$sql = "select * from san_pham where ma_loai_cha=15 limit $vt,$limit";
			else 
